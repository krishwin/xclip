...
class counitxDataset(VideoDataset):
    def __init__(self, ann_file, pipeline, labels_file, start_index=0, **kwargs):
        super().__init__(ann_file, pipeline, labels_file,start_index=start_index, **kwargs)
        self.labels_file = labels_file
    def load_annotations(self):
        video_infos = []
        i=0
        with open(self.ann_file, 'r',encoding='utf-8-sig') as fin:
            for line in fin:
                ...
                if self.data_prefix is not None:
                    filename = osp.join(self.data_prefix, filename)
                video_infos.append(dict(filename=filename, label=label,start=start,end=end,count=count, tar=self.use_tar_format))
        return video_infos
...
def countix_collate(batch, samples_per_gpu=1):
    max_len = 0 
...
def build_counitxdataloader(config):
    scale_resize = int(256 / 224 * config.DATA.INPUT_SIZE)
    ...
    train_pipeline = [
       ...
       # dict(type='SampleFrames', clip_len=1, frame_interval=1, num_clips=config.DATA.NUM_FRAMES),
        ...
        dict(type='Resize1', h=config.DATA.INPUT_SIZE,w=config.DATA.INPUT_SIZE),
        #dict(type='Flip', flip_ratio=0.5),
        #dict(type='ColorJitter', p=config.AUG.COLOR_JITTER),
        #dict(type='GrayScale', p=config.AUG.GRAY_SCALE),
        #dict(type='Normalize', **img_norm_cfg),
        #dict(type='FormatShape', input_format='NCHW'),
        dict(type='ToTensor1'),
        dict(type='Normalize1', mean=0.5,std=0.5),
        dict(type='Collect', keys=['imgs', 'label','start','end','count'], meta_keys=[]),
        dict(type='ToTensor', keys=['imgs', 'label','start','end','count']),
    ]
    train_data = counitxDataset(ann_file=config.DATA.TRAIN_FILE, data_prefix=config.DATA.ROOT,
                              labels_file=config.DATA.LABEL_LIST, pipeline=train_pipeline)
   #num_tasks = dist.get_world_size()
    #global_rank = dist.get_rank()
    #sampler_train = torch.utils.data.DistributedSampler(
