...
def main(config): 
    ...
    model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[config.LOCAL_RANK] if torch.cuda.is_available() else None, broadcast_buffers=False, find_unused_parameters=False)
...
def train_one_epoch(epoch, model, criterion, optimizer, lr_scheduler, train_loader, text_labels, config, mixup_fn,scaler):
    ...
    texts = text_labels.to('cuda' if torch.cuda.is_available() else 'cpu' ,non_blocking=True)#text_labels.cuda(non_blocking=True)
    
    for idx, batch_data in enumerate(train_loader):

        images = batch_data["imgs"].to('cuda' if torch.cuda.is_available() else 'cpu' ,non_blocking=True)#.cuda(non_blocking=True)
        label_id = batch_data["label"].to('cuda' if torch.cuda.is_available() else 'cpu' ,non_blocking=True)#.cuda(non_blocking=True)
...
def validate(val_loader, text_labels, model, config):
    ...
    with torch.no_grad():
        text_inputs = text_labels.to('cuda' if torch.cuda.is_available() else 'cpu')#.cuda()
        ...
        for idx, batch_data in enumerate(val_loader):
            ...
            tot_similarity = torch.zeros((b, config.DATA.NUM_CLASSES)).to('cuda' if torch.cuda.is_available() else 'cpu')#.cuda()
            for i in range(n):
                image = _image[:, i, :, :, :, :] # [b,t,c,h,w]
                label_id = label_id.to('cuda' if torch.cuda.is_available() else 'cpu' ,non_blocking=True)#.cuda(non_blocking=True)
                image_input = image.to('cuda' if torch.cuda.is_available() else 'cpu' ,non_blocking=True)#.cuda(non_blocking=True)
...
if __name__ == '__main__':
    ...
    torch.distributed.init_process_group(backend='nccl' if torch.cuda.is_available() else 'gloo', init_method='env://', world_size=world_size, rank=rank)
