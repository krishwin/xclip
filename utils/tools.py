...
class AverageMeter:
    ...
    def sync(self):
        ...
        val = torch.tensor(self.val).to('cuda' if torch.cuda.is_available() else 'cpu' ,non_blocking=True)#.cuda()
        sum_v = torch.tensor(self.sum).to('cuda' if torch.cuda.is_available() else 'cpu' ,non_blocking=True)#.cuda()
        count = torch.tensor(self.count).to('cuda' if torch.cuda.is_available() else 'cpu' ,non_blocking=True)#.cuda()
